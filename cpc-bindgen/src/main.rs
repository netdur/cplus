...
fn emit_c_header(header: &str, clang_args: &[String], llp64: bool) -> Result<String, String> {
    let mut cmd = Command::new("clang");
...
/// LP64 of Linux/macOS (64-bit). Ask clang for the effective target triple
/// (honoring an explicit `-target`/`--target=` in the forwarded args) so
/// DWORD/LONG widths in generated structs and params match the ABI.
fn target_is_llp64(clang_args: &[String]) -> bool {
    let mut cmd = Command::new("clang");
    cmd.arg("-dumpmachine");
    let mut i = 0;
    while i < clang_args.len() {
        if clang_args[i] == "-target" {
            if let Some(t) = clang_args.get(i + 1) {
                cmd.arg("-target").arg(t);
            }
            i += 2;
            continue;
        }
        if let Some(t) = clang_args[i].strip_prefix("--target=") {
    ...
    match cmd.output() {
        Ok(o) if o.status.success() => {
            let triple = String::from_utf8_lossy(&o.stdout).to_lowercase();
            triple.contains("windows") || triple.contains("mingw")
        }
        _ => false,
    }
}
...
fn generate_c_package(
    ...
    let llp64 = target_is_llp64(&clang_args);
    let mut modules: Vec<String> = Vec::new();
    for header in headers {
        let src = emit_c_header(header, &clang_args, llp64)?;
...
struct Emitter {
    ...
    /// LLP64 target (Windows): `long`/`unsigned long` are 32-bit, not the
...
impl Emitter {
    fn new(header_path: &str, llp64: bool) -> Self {
    ...
    fn map_type(&self, qt: &str) -> Result<String, String> {
        map_c_type_to_cplus(&self.expand(qt, 16), &self.seen_records, self.llp64)
    }
    ...
    fn emit_record(&mut self, decl: &serde_json::Value) {
        ...
        for field in &inner {
            ...
            // C11 anonymous struct/union members have no name; `sanitize_ident("")`
            // yields a bare `_`, which is not a spellable C+ field identifier. Give
    ...
    fn emit_typedef(&mut self, decl: &serde_json::Value) {
        ...
        if let Some((lhs, rhs)) = qual_type.rsplit_once(' ') {
            if rhs == name
                ...
                && !lhs.contains("union ")
            {
                if let Ok(cplus_ty) = map_c_type_to_cplus(lhs.trim(), &self.seen_records, self.llp64) {
                    self.out
...
fn parse_fn_qual_type(qt: &str) -> Option<(String, Vec<String>, bool)> {
    ...
    let open = qt.find('(')?;
    // Depth-match the paren that closes the parameter list, starting from the
    // first `(`. Using `rfind(')')` is wrong on Windows: every function's
    // qualType carries a trailing calling-convention attribute
    // (`int (HWND, ...) __attribute__((stdcall))`), so `rfind` returns the
    // attribute's inner `)` and folds `__attribute__((stdcall)` into the last
    // param — skipping the entire user32/gdi32 surface. Everything after the
    // matched close (the attribute) is not part of the signature.
    let close = {
        let mut depth = 0i32;
        let mut found = None;
        for (idx, ch) in qt[open..].char_indices() {
            match ch {
                '(' => depth += 1,
                ')' => {
                    depth -= 1;
...
fn map_c_type_to_cplus(
    ...
    if let Some(p) = c_ty.find("__vector_size__(") {
        ...
        if !n.is_empty() && !elem.is_empty() {
            if let Ok(es) = map_c_type_to_cplus(elem, complete, llp64) {
                if matches!(
                    es.as_str(),
    ...
    if let Some(inner) = s.strip_suffix('*') {
        ...
        return match map_c_type_to_cplus(inner, complete, llp64) {
            Ok(t) => Ok(format!("*{t}")),
    ...
    Ok(match s {
        ...
        // Single `long` (C `LONG`/`DWORD` after typedef expansion) is 32-bit on
        // LLP64 (Windows) and 64-bit on LP64 (Linux/macOS); `long long` is always
        // 64-bit. Getting this wrong corrupts every DWORD/LONG struct field.
        "long" | "signed long" | "long int" | "signed long int" => {
            if llp64 { "i32".to_string() } else { "i64".to_string() }
        }
        "unsigned long" | "unsigned long int" => {
            if llp64 { "u32".to_string() } else { "u64".to_string() }
        }
...
mod tests {
    ...
    fn scalar_map() {
        ...
        assert_eq!(map_c_type_to_cplus("int", &c, false).unwrap(), "i32");
        assert_eq!(map_c_type_to_cplus("unsigned int", &c, false).unwrap(), "u32");
        assert_eq!(map_c_type_to_cplus("size_t", &c, false).unwrap(), "usize");
        assert_eq!(map_c_type_to_cplus("char *", &c, false).unwrap(), "*i8");
        assert_eq!(map_c_type_to_cplus("const char *", &c, false).unwrap(), "*i8");
        assert_eq!(map_c_type_to_cplus("void", &c, false).unwrap(), "()");
        assert_eq!(map_c_type_to_cplus("uint32_t **", &c, false).unwrap(), "**u32");
    }

    // LP64 (Linux/macOS): `long` is 64-bit. LLP64 (Windows): `long` is 32-bit,
    // but `long long` stays 64-bit either way. DWORD/LONG/ULONG expand to these.
    #[test]
    fn long_width_follows_data_model() {
        let c: std::collections::HashSet<String> = std::collections::HashSet::new();
        // LP64
        assert_eq!(map_c_type_to_cplus("long", &c, false).unwrap(), "i64");
        assert_eq!(map_c_type_to_cplus("unsigned long", &c, false).unwrap(), "u64");
        // LLP64
        assert_eq!(map_c_type_to_cplus("long", &c, true).unwrap(), "i32");
        assert_eq!(map_c_type_to_cplus("unsigned long", &c, true).unwrap(), "u32");
        // long long is 64-bit in both models
        assert_eq!(map_c_type_to_cplus("long long", &c, true).unwrap(), "i64");
        assert_eq!(map_c_type_to_cplus("unsigned long long", &c, true).unwrap(), "u64");
        assert_eq!(map_c_type_to_cplus("long long", &c, false).unwrap(), "i64");
    }
    ...
    fn simd_vector_map() {
        ...
        let v = |s: &str| map_c_type_to_cplus(s, &c, false).unwrap();
    ...
    fn function_pointer_map() {
        let c: std::collections::HashSet<String> = std::collections::HashSet::new();
        let f = |s: &str| map_c_type_to_cplus(s, &c, false).unwrap();
    ...
    // Every Windows SDK function's qualType carries a trailing calling-convention
    // attribute (`__attribute__((stdcall))`). The param list's close paren must be
    // depth-matched from the first `(`, not `rfind`'d, or the attribute folds into
    // the last param and the whole win32 surface is skipped.
    #[test]
    fn fn_qual_type_strips_trailing_stdcall_attr() {
        let (ret, params, var) =
            parse_fn_qual_type("int (HWND, LPCSTR, LPCSTR, UINT) __attribute__((stdcall))").unwrap();
        assert_eq!(ret, "int");
        assert_eq!(params, vec!["HWND", "LPCSTR", "LPCSTR", "UINT"]);
        assert!(!var);
    }
    ...
    fn in_header_filter_is_sticky_across_elided_files() {
        let e = Emitter::new("/usr/include/cblas.h", false);
