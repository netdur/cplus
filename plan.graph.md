...
- **Phases 4b–7 open.** Next: type references (uses of a type in signatures, fields, lets, casts, generic args) and value references (const/static/fn-as-value), then `type-at` and `context` (which `cpc query` reports as "not available in this build", exiting non-zero). The receiver-type precision upgrade (option B, sema-retained resolution) remains the path to driving the call `unresolved` count toward zero (§12).
