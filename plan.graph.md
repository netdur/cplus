...
- **Phase 4 shipped (references).** `cpc query refs <symbol>` returns resolved use sites with precise `file:line:col` and the enclosing item. Coverage (`scope: "calls,types"`): **call sites** (4a — reusing Phase 3 resolution) and **named-type uses** (4b — in signatures, struct fields, enum payloads, const/static/alias types, let annotations, casts, and struct/enum-literal construction, resolved against the type index). **Value references** (const/static/fn-as-value) are the remaining slice, so an agent still greps for those. Unit + e2e tested.
- **Composite `context` shipped.** `cpc query context <fn>` returns the one-shot edit pack — the function's node (signature + location), its `callers`, `callees`, the **types it touches** (`type_refs`), and the count of unresolved calls inside it. Composes the existing edges/references, no new analysis. Unit + e2e tested.
- **`type-at` shipped (partial, honest).** `cpc query type-at FILE:LINE:COL` returns the locally-known type at a position — resolving **parameters, fields, typed locals, `self`, and their identifier uses** from a sparse type-spot map, picking the narrowest containing span. Inferred expressions (a call result, an arithmetic temporary) report "no locally-typed node" rather than guessing — the full version needs sema's per-expression types retained (§12, option B). Unit + e2e tested; verified on `vendor/uuid` (`type-at …:70:18` → `str` parameter).
**Every `cpc query` kind now answers.** The remaining work is depth and delivery, not new query kinds:
