...
- **Phase 4a shipped (call-site references).** `cpc query refs <symbol>` returns resolved use sites with precise `file:line:col` and the enclosing item, reusing the Phase 3 call resolution. Each result carries `scope: "call-sites"`, the honesty signal that this slice resolves call references only — type and value references (Phase 4b) are not yet included, so an agent still greps for those. Unit + e2e tested.
...
- **Phases 4b–7 open.** Next: type references (uses of a type in signatures, fields, lets, casts, generic args) and value references (const/static/fn-as-value) to broaden `refs` beyond call sites; then `type-at` (the one remaining query `cpc query` reports as "not available in this build", exiting non-zero). The receiver-type precision upgrade (option B, sema-retained resolution) remains the path to driving the call `unresolved` count toward zero (§12); the resident mode (§3) and MCP adapter (Phase 7) are the agent-loop delivery.
