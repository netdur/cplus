...
- **Composite `context` shipped.** `cpc query context <fn>` returns the one-shot edit pack — the function's node (signature + location), its `callers` and `callees`, and the count of unresolved calls inside it — composing the existing edges, no new analysis. Unit + e2e tested.
- **Phases 5–7 open.** Remaining: value references (const/static/fn-as-value) to finish `refs`; `type-at` (the one query `cpc query` still reports as "not available in this build", exiting non-zero), which needs sema's per-expression types retained; the receiver-type precision upgrade (option B, sema-retained resolution) to drive the call `unresolved` count toward zero (§12); and the resident mode (§3) + MCP adapter (Phase 7) as the agent-loop delivery.
