...
## Generated bindings are exempt

A package whose `Cplus.toml` header says `Auto-generated by cpc-bindgen` ships
**no README and no `docs/`**, and that is deliberate rather than an omission.
`appkit`, `mapkit`, `quartzcore`, `avfoundation`, `coreml`, `vision`,
`foundation` and `corelocation` all follow it.

The reasons: the surface is the framework's, already documented by its vendor;
it is regenerated wholesale, so prose beside it rots on the next run; and the
`reproduce = "cpc-bindgen --framework X --prefix P --merge"` line in the
manifest header is the only documentation that stays true — it says exactly how
to rebuild what you are reading.

**A hand-written `*_ext.cplus` beside generated output is the exception that
still needs comments**, because it is the part a regeneration will not
recreate. Document it at the definitions, in the file, the way
`appkit_ext.cplus` and `corelocation_ext.cplus` do — not in a `docs/` folder
that would sit next to a generated file it does not describe.
