#!/bin/sh
# Regenerate camera.dex from java/. Needs a JDK and the Android SDK.
#
# THE DEX IS A COMMITTED BUILD ARTIFACT. Editing java/cplus/camera/*.java
...
javac -source 8 -target 8 -nowarn -classpath "$AJ" -d classes java/cplus/camera/*.java
"$BT/d8" --release --lib "$AJ" --min-api 26 --output . classes/cplus/camera/*.class
mv classes.dex camera.dex
rm -rf classes
...
SRC=src/camera_backend_android.cplus
SIZE=$(stat -f%z camera.dex 2>/dev/null || stat -c%s camera.dex)
echo "camera.dex: $SIZE bytes"
