...
import "gtk/gtk" as gtk;           // base GTK widgets they hold
...
- `window`: `Window` (AdwApplicationWindow) and `PlainWindow` (AdwWindow) — use
  `set_content` (not `set_child`); host a `ToolbarView`. Geometry/lifecycle is
  inherited from `gtk::Window` (reachable via `gtk::Window::from_raw(win.raw())`).
