#!/bin/sh
# Regenerate biometrics.dex from java/. Needs a JDK and the Android SDK.
#
# THE DEX IS A COMMITTED BUILD ARTIFACT. Editing java/cplus/biometrics/*.java
...
javac -source 8 -target 8 -nowarn -classpath "$AJ" -d classes java/cplus/biometrics/*.java
"$BT/d8" --release --lib "$AJ" --min-api 26 --output . classes/cplus/biometrics/*.class
mv classes.dex biometrics.dex
rm -rf classes
...
SRC=src/biometrics_backend_android.cplus
SIZE=$(stat -f%z biometrics.dex 2>/dev/null || stat -c%s biometrics.dex)
echo "biometrics.dex: $SIZE bytes"
