...
- `drag`: drag-and-drop destination. `create_drag_destination_view(frame, entered_imp, perform_imp)` synthesizes an `NSView` that accepts drops; `register_for_string_drops`, `dragged_string_ns`, and `NSDragOperation` constants (`drag_op_copy`/`drag_op_move`/…). Import directly.
