...
| Module | Surface |
|---|---|
| `cuda/runtime` | `device_count()`, `set_device()`, `synchronize()`, `error_string()`, `CudaError` |
| `cuda/buffer`  | `alloc()`, `DeviceBuffer { dptr, size }` (Drop = `cudaFree`), `copy_from_host` / `copy_to_host` |
| `cuda/cublas`  | `handle_new()`, `Handle` (Drop = `cublasDestroy`), `sgemm`, `sgemv`, `Op` |
...
guard let result::Result[buf::DeviceBuffer, rt::CudaError]::Ok(da) = buf::alloc(16 as usize) else { return 1 as i32; };
let dA: buf::DeviceBuffer = da;            // owned; freed on scope exit
// (same for dB, dC)

let _ = dA.copy_from_host(hostA, 16 as usize);
let _ = dB.copy_from_host(hostB, 16 as usize);

guard let result::Result[blas::Handle, rt::CudaError]::Ok(hh) = blas::handle_new() else { return 2 as i32; };
let h: blas::Handle = hh;

let st: i32 = h.sgemm(
    blas::Op::N, blas::Op::N,
    2 as i32, 2 as i32, 2 as i32,
    1.0f32, dA.dptr, 2 as i32, dB.dptr, 2 as i32,
    0.0f32, dC.dptr, 2 as i32);
let _ = rt::synchronize();
let _ = dC.copy_to_host(hostC, 16 as usize); // hostC == [19, 43, 22, 50]
```

Pass `DeviceBuffer.dptr` (a device pointer) to `sgemm`/`sgemv` — that keeps
