...
- **System Back addresses the mobile app's window navigator.**
  `App::run` sets up the initial window and returns to the Activity-owned loop.
  The runtime binds the native Back dispatch to that window's `pop()`. When
  no Back entry exists, the adapter returns false and the platform handles
  leaving the root. Applications using registered routes need no hook of
  their own. `nav::pop` / `set_pop_fn` remain this native adapter seam;
  application content commands use a retained navigator.

  The window owns retained history through `facet/navigation`. There is no
  app-wide stack, `Show` mode, or window-opening fallback. Android refuses an
  additional live app window. The earlier Galaxy Fold gesture measurement
  (2026-09-06) covered the previous stack implementation; mobile builds passed
  for the redesign, but that does not repeat the device gesture validation.
  See [Facet navigation](../facet/docs/navigation.md#desktop-and-mobile).
