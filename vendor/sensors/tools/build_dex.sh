#!/bin/sh
# Regenerate sensors.dex from java/. Needs a JDK and the Android SDK.
#
# THE DEX IS A COMMITTED BUILD ARTIFACT. Editing java/cplus/sensors/*.java
...
javac -source 8 -target 8 -nowarn -classpath "$AJ" -d classes java/cplus/sensors/*.java
"$BT/d8" --release --lib "$AJ" --min-api 26 --output . classes/cplus/sensors/*.class
mv classes.dex sensors.dex
rm -rf classes
...
SRC=src/sensors_backend_android.cplus
SIZE=$(stat -f%z sensors.dex 2>/dev/null || stat -c%s sensors.dex)
echo "sensors.dex: $SIZE bytes"
