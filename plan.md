...
- Phase 2 — ⏳ pending (skeleton only — see Phase 2 block)
- Phase 3A — ✅ shipped (bitshifts + byte-swap intrinsics); 3B/3C pending
...
└── src/                      ← all importable C+ code lives here
    ...
    │   └── baz.cplus
    └── lib/                  ← bundled prebuilt artifacts (optional)
...
| Mode | `[link]` | `src/lib/` | Example |
|---|---|---|---|
| **Pure C+ source** | no | no | `stdlib`, with `src/result.cplus` containing `pub enum Result[T, E]` |
| **System-libs only** | yes | no | `appkit`, with `src/appkit.cplus` declaring `extern fn` into Cocoa + `[link] frameworks=["Cocoa"]` |
| **Bundled artifact + transitive system libs** | usually yes | yes | `curl_bindings`, with `src/curl.cplus` + `src/lib/<triple>/curl.a` + `[link] libs=["z"]` |
| **Mixed source + artifact (per-module)** | optional | yes (some modules) | Source for portable code, artifact for hot loops |
...
`cpc build` walks the consumer's `[dependencies]`, opens each `vendor/<name>/Cplus.toml`, collects all `[link].frameworks` / `[link].libs` plus any matching `src/lib/<host-triple>/*.a` artifacts, and splices them into the clang link line. The consumer's source uses `import "<name>/<module>"` with no extension; the resolver appends `.cplus` and prepends `src/` to locate the file inside the vendor package.

#### Slice 2A — Unified `Cplus.toml` schema · `[dependencies]` and `[link]`
...
#### Slice 2B — Import resolver: vendor lookup + strict path-shape rule + no extension
...
#### Slice 2C — Build driver: package-aware linking · host triple + artifacts + `[link]` propagation
...
- For each declared dep:
  ...
  - If `vendor/<name>/src/lib/` exists: locate `src/lib/<host-triple>/`. For each `.a` inside, splice into the link args. If the dir exists but `<host-triple>` is missing, fail with **E0856** "package `<name>` ships artifacts for {list of triples}, host is `<host>`".
  - If `vendor/<name>/src/lib/` doesn't exist at all: source-only package, no artifact step.
- Order: consumer's own `[[bin]] frameworks`/`libs` first, then each dep's `[link]` and artifacts (in `[dependencies]` declaration order).
- `cpc test` and `cpc build` use the same link path. `--emit-ll-project` already exists; it doesn't need link args (no link step) but should still surface the dep walk so E0850/E0854/E0855 fire before codegen.
...
**Tests:**
- Unit: `[link]` parsing produces the right `Vec<String>` for frameworks and libs.
...
- E2E: a package shipping `vendor/tiny_artifact/src/lib/<host-triple>/tiny.a` with one C function declared via `extern fn` in `tiny_artifact.cplus` links and runs. (Generate the `.a` inside the test by shelling to clang on a tiny C source file in the test harness setup — *not* via cpc.)
- E2E negative: missing arch dir → E0856; package `Cplus.toml` name/dir mismatch → E0855.

**Exit:** A consumer that depends on a pure-system-libs package (appkit) and a bundled-artifact package (tiny_artifact) builds, links, and runs end-to-end on the host triple.

#### Slice 2D — Migrate existing imports + ship two smoke-test packages
...
#### Slice 3B — Reference program library · est. 1 week
