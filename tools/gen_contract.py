...
// DID THE SET MOVE — the question every backend asks before firing
// `on_item_selected`, which is a selection-CHANGED verb: a click landing on
// the selection it already had is not a change, and firing anyway makes an
// application re-select what it has, rebuilding a pane for nothing.
//
// ORDER IS PART OF IT. The first element is `selected_index`, so a selection
// reordered to the same members IS a change.
//
// It lives here because all five backends need it and two had already written
// it out by hand. Free rather than a method: both sides are read through
// `#addr_of` on a props field, and a method on `this` would want them moved.
fn same_indexes(a: *IndexList, b: *IndexList) -> bool {
    if a == (0 as *IndexList) || b == (0 as *IndexList) { return false; }
    let n: usize = { (*a).count() };
...
def emit_control(row_type, merged):
    ...
    if mod in ROW_SOURCE:
        ...
        o.append("    // WHICH — and it is the FIRST row of the set below, not the\n")
        o.append("    // whole of it. The ledger carries SelectedItem and SelectedItems\n")
        o.append("    // and Stage 1 dropped both as MODEL, so the switch had a third\n")
        o.append("    // position (`Multiple`) and nothing to hold what it switched on.\n")
        o.append("    // -1 is none, and writing it empties the set but for that row.\n")
        o.append(f"    fn set_selected_index(this, v: i64) -> {cur} {{\n")
        o.append(f"        let p: *props::{props} = this._props();\n")
        o.append(f"        if p == (0 as *props::{props}) {{ return this; }}\n")
        o.append("        { (*p).selected_index = v };\n")
...
fn split(
    ...
    role_leading: m_split::PaneRole = m_split::PaneRole::Content,
    role_trailing: m_split::PaneRole = m_split::PaneRole::Content,
