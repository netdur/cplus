# v0.0.12 — shipped (closed 2026-06-01)
...
- **Phase 1 — `#[no_alloc]`** ✅ (hardened) — call-graph walk rejects libc allocators + unmarked user callees + unknown externs (E0901). Rejects **string interpolation** (lowers to a `__string_concat` malloc). The shared body walker is an extensible `BodyEffects { calls, interps, let_tys }`. **Method-dispatch hole closed** (the one open *soundness* gap): `recv.method()` calls inside a `#[no_alloc]`/`#[no_block]` body are now enforced — the dispatched method (or `to_string`) must carry the same contract, else E0901/E0907. Done precisely at typecheck time in `check_method_call`, where the receiver type is known, so it's the *actual* method's verdict, not a name-collision guess: a `(source-type, method)` → `(no_alloc, no_block)` map (`collect_method_contracts`, keyed on the source type via `generic_origin` so `Vec[i32]`→`Vec`) is consulted, with `current_fn_no_alloc`/`current_fn_no_block` flags threaded through `check_function_body`. Validated no false positives (vendor/rt, realtime_audio demo, full e2e all green) + 4 new e2e tests. *Open (deferred):* **drop-glue** — a `Drop` impl whose destructor allocates, run implicitly at scope exit of a no_alloc body. Narrow (destructors free, not allocate) and needs ownership analysis to know which values drop; documented, not yet enforced.
