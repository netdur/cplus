...
1. `m3.cplus` compiles and returns 2.
2. Move-join check: a program that moves a value in a diverging-via-match then-branch and
   uses it after the `if` must compile (add as an e2e positive test).
3. Existing divergence tests: grep e2e.rs for E0302/diverge tests and run the full suites.
